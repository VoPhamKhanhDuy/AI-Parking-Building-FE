...
public class ParkingSessionServiceTests
{
        ...
        Build()
    {
        ...
        var svc = new ParkingSessionService(sessions, tickets, slots, uow, clock);
        return (svc, sessions, tickets, slots, uow);
    }

    private static (ParkingTicket ticket, ParkingSlot slot, Vehicle vehicle)
        SeedAvailableTicket(InMemoryRepository<ParkingTicket> tickets, InMemoryRepository<ParkingSlot> slots)
    {
    ...
    {
        var (svc, sessions, tickets, slots, uow) = Build();
        var (ticket, slot, _) = SeedAvailableTicket(tickets, slots);
    ...
    {
        var (svc, _, tickets, slots, _) = Build();
        var (ticket, slot, _) = SeedAvailableTicket(tickets, slots);
    ...
    {
        var (svc, sessions, tickets, slots, _) = Build();
        var (ticket, slot1, _) = SeedAvailableTicket(tickets, slots);
    ...
    public async Task StartAsync_throws_409_when_slot_already_has_active_session_even_if_reserved()
    {
        ...
        var (svc, sessions, tickets, slots, _) = Build();
        var (ticket1, slot, _) = SeedAvailableTicket(tickets, slots);
    ...
    public async Task EndAsync_re_fetches_slot_when_navigation_null_and_404s_if_missing()
    {
        ...
        var (svc, sessions, tickets, slots, _) = Build();
        var (ticket, _, _) = SeedAvailableTicket(tickets, slots);
        var session = new ParkingSession
        {
            Id = Guid.NewGuid(),
            TicketId = ticket.Id,
