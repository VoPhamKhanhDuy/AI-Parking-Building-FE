...
import { safeArray } from '../../core/utils/apiShapers'
...
function CheckinSuccessPage() {
  ...
  const slotCode = session.assignedSlot || session.slotCode || session.selectedSlotId || 'B2-18'
  const floor = session.selectedFloor || (slotCode.includes('2') || slotCode.startsWith('C') || slotCode.startsWith('L2') ? 'Floor 2' : 'Floor 1')
  const entryTime = formatSessionTime(session.entryTime)
  const nextSteps = safeArray(getCheckinNextSteps())
  ...
  return <MainLayout><div className="checkin-page">
    ...
    <div className="checkin-layout"><main className="checkin-main">
      <section className="success-card"><div className="success-card-title"><span><b className="material-symbols-outlined">check_circle</b>Check-in completed</span><em><i />Active session</em></div><div className="success-content"><div className="assigned-slot"><small>Assigned slot</small><strong>{slotCode}</strong></div><div><small>Ticket code</small><strong>{session.ticketCode}</strong><small>Method</small><span>{session.method}</span></div><div><small>Entry time</small><strong>{entryTime}</strong><small>Match score</small><span>{session.matchScore}</span></div></div></section>
      ...
      <section className="info-card"><h2><span className="material-symbols-outlined">location_on</span>Slot assignment detail</h2><dl><InfoRow label="Assigned slot" value={slotCode} accent="blue"/><InfoRow label="Floor" value={floor}/><InfoRow label="Slot status" value="Occupied" accent="green"/><InfoRow label="Assignment method" value={session.method}/><InfoRow label="AI match score" value={session.matchScore} accent="green"/></dl><p>AI log status: <b className="green">Saved</b></p></section>
    </main>
