...
import { initialAuditKPIs, initialAuditRecords, initialSecurityEvents, fetchAuditLogs } from './auditLogsService'
...
function AuditLogsPage() {
  ...
  const handleMarkReviewed = () => {
    showToast(`Đã đánh dấu nhật ký kiểm toán ${selectedLog.id} là Đã kiểm duyệt (Reviewed).`, 'success')
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="glass-card p-4 rounded-lg border-l-4 border-green-500 bg-white">
            <div className="flex items-center justify-between mb-1">
                          ...
                          <td className="px-6 py-3">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${log.result === 'Success' ? 'bg-green-50 text-green-700' :
                                log.result === 'Failed' ? 'bg-red-50 text-red-700' :
                                  log.result === 'Pending' ? 'bg-slate-100 text-slate-600' :
